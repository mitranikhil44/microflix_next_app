...
const Movies = (props) => {
  ...
  const fetchMoreData = async () => {
    try {
      ...
      const data = await fetch(`https://microflix-next-app.vercel.app/api/blogs/?category=hollywood/movies&skip=${updatedSkip}&limit=${limit}`);
...
export async function getServerSideProps() {
  try {
    const data = await fetch(`https://microflix-next-app.vercel.app/api/blogs/?category=hollywood/movies&skip=0&limit=12`);
